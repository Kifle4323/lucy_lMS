...
export function registerMaterialRoutes(router) {
    // Get materials for a course
    router.get('/courses/:courseId/materials', authRequired, async (req, res) => {
    ...
    router.post('/courses/:courseId/materials', authRequired, requireRole(['TEACHER']), async (req, res) => {
        const params = z.object({ courseId: z.string() }).parse(req.params);
        const body = z.object({
    ...
    router.put('/materials/:materialId', authRequired, requireRole(['TEACHER']), async (req, res) => {
    ...
    router.delete('/materials/:materialId', authRequired, requireRole(['TEACHER']), async (req, res) => {
        const params = z.object({ materialId: z.string() }).parse(req.params);
        const user = req.user;
        const material = await prisma.material.findUnique({ where: { id: params.materialId } });
        if (!material) {
